...
#        'state': fields.selection(selection=_get_state, string='State',
#                    required=True),
...
#           help=' * The \'Draft\' state is used when a user is encoding a
#                    new and unconfirmed Invoice. \
#           \n* The \'Pro-forma\' when invoice is in Pro-forma state,
#                    invoice does not have an invoice number. \
#           \n* The \'Open\' state is used when user create invoice,a invoice
#       number is generated.Its in open state till user does not pay invoice. \
                ...
                for move in account_move_obj.browse(cr, uid, move_ids):
                    if not move.journal_id.reopen_posted:
                        raise except_orm(_('Error !'),
                                         _('You can not reopen invoice of '\
                                           'this journal [%s]! You need to '\
                                           'set Allow Update Posted Entries'\
                                           'first') % (move.journal_id.name))

            if i['payment_ids']:
                pay_ids = account_move_line_obj.browse(cr, uid,
                                                       i['payment_ids'])
                for move_line in pay_ids:
                    if move_line.reconcile_id or\
                        (move_line.reconcile_partial_id and\
                         move_line.reconcile_partial_id.line_partial_ids):
                        raise except_orm(_('Error !'),
                                         _('You can not reopen an invoice '\
                                           'which is partially paid! You '\
                                           'need to unreconcile related'\
                                           'payment entries first!'))
            self.write(cr, uid, i['id'], {'state': 'draft'})
            workflow.trg_delete(uid, 'account.invoice', i['id'], cr)
            workflow.trg_create(uid, 'account.invoice', i['id'], cr)
            ...
            for report in report_xml_obj.browse(cr, uid, report_ids):
                if report.attachment:
                    ...
                    if eval(aname):
                        ...
                        dmn = [('res_model', '=', 'account.invoice'),
                               ('datas_fname', '=', aname),
                               ('res_id', '=', inv.id)]
                        attachment_ids = attachment_obj.search(cr, uid, dmn)
                        for a in attachment_obj.browse(cr, uid,
                                                       attachment_ids):
                            vals = {
                                'name': a.name.replace('.pdf', now + '.pdf'),
                                'datas_fname':
                            a.datas_fname.replace('.pdf.pdf', now + '.pdf.pdf')
                                   }
        ...
        if move_ids:
            for move in account_move_obj.browse(cr, uid, move_ids):
                ...
                if move.journal_id.entry_posted:
                    raise except_orm(_('Error !'),
                                     _('You can not reopen an invoice '\
                                       'if the journal is set to skip draft!'))
                ...
                account_move_obj.button_validate(cr, uid,
                                                 [move_copy_id], context=None)
                _logger.debug('FGF reopen move_copy_id validated')
                # reconcile
                amr_obj = self.pool.get('account.move.reconcile')
                r_id = amr_obj.create(cr, uid, {'type': 'auto'})
                _logger.debug('FGF reopen reconcile_id %s' % (r_id))
                mv_domain = [('move_id', 'in', [move_copy_id, move.id])]
                line_ids = account_move_line_obj.search(cr, uid, mv_domain)
                ...
                for ltr in account_move_line_obj.browse(cr, uid, line_ids):
                    ...
                    (ltr.partner_id.property_account_payable.id,
                     ltr.partner_id.property_account_receivable.id):
                        lines_to_reconile.append(ltr.id)
                account_move_line_obj.write(cr, uid, lines_to_reconile,
                                            {'reconcile_id': r_id})
